...
Esta es una aplicación de escritorio de **Doblaje Automático de Video** (Video Dubbing) escrita en Python. Su principal propósito es tomar un video en un idioma origen (por ejemplo, Inglés), transcribir su audio, traducirlo estructuradamente a otro idioma (Español) conservando milimétricamente las marcas de tiempo (timestamps), sintetizar nueva voz y mezclarla encima del video original.
...
2. **Interfaz Gráfica (GUI)**: Se utiliza `customtkinter`. Todo código visual debe ser moderno y responsivo.
...
Las fases completas del pipeline se orquestan centralmente en la GUI principal `gui/app.py`:
...
4. `translator.py`: Traducción LLM (`google-genai`).
5. `tts.py`: Audio neuronal emparejado (`edge-tts` + `pydub`).
6. `assembler.py`: Mezclador del video final atenuando los sonidos originales (`ffmpeg`).
...
- **Rutas Relativas al Workspace**: Siempre usa la carpeta temporal universal (`os.path.join(os.getcwd(), "temp")`) para todas las fases, y deja el resultado final en el directorio padre `os.getcwd()`.
